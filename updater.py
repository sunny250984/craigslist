...
class Updater(QThread):
    ...
    def quit(self):
        # Clean up
        if self.driver != None:
            logging.info('Cleaning up Chromedriver')
            self.driver.close()
            self.driver.quit()
    ...
    def search_page2(self, query_url, db):
        ...
        #     EC.presence_of_element_located((By.CLASS_NAME, "nearby-separator"))
        # )
        # Parse the webpage and find the search results
        soup = BeautifulSoup(self.driver.page_source, 'html.parser')
        ...
        if results is None:
            ...
            if self.retry >= 3:
                raise Exception(f'Can\'t get results in {self.retry} retries')
            return []
        ...
        if stop_tag != None:
            ...
            rows = stop_tag.findAllPrevious('div', {'class': 'cl-search-result', 'data-pid': True})
            
        else:
            # rows = results.findAll('li', {'class': 'cl-search-result'})
            rows = results.findAll('div', {'class': 'cl-search-result', 'data-pid': True})

        # Go through each search result
        for result in rows:
            # Extract the different information that we need
            # link = result.find('a', {'class': 'titlestring'})
            link = result.find('a', {'class': 'posting-title'})
            name = link.text
            url = link.get('href')
            if 'sandiego.craigslist.org' not in url:
                continue
            # id = re.findall('^.+/(\d+)\.html$', url)
            id = result.get('data-pid')
